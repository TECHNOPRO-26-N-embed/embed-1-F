...
3. 入力が変化しており、かつ 50ms 以上経過していれば確定入力として buttonPrevState を更新する
4. 前回が非押下（HIGH）かつ今回が押下（LOW）のときのみ buttonPressEvent = true にする
5. それ以外は buttonPressEvent = false にする
6. 入力変化がない場合も buttonPressEvent = false にする
7. buttonPressEvent を戻り値として返す
...
3. 50ms以上経過していれば「状態変化あり」と判定し、buttonPrevState を更新
4. 50ms未満ならノイズ（チャタリング）とみなし、押下イベントは発生させない
